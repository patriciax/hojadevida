<script setup>
import { ArrowDownIcon, ArrowDownLeftIcon, ArrowDownTrayIcon, Cog6ToothIcon, DocumentTextIcon, LinkIcon, PrinterIcon, ShareIcon } from '@heroicons/vue/24/solid'
...
<template>
  <div class="lg:hidden block fixed z-50 w-full h-16  movil-share -translate-x-1/2 bg-white rounded-t-md bottom-0 left-1/2 shadow-xl">
    <div class="grid h-full max-w-lg grid-cols-5 mx-auto">
      ...
      <button data-tooltip-target="tooltip-home" type="button" class="text-gray-500 inline-flex flex-col items-center justify-center px-5 rounded-s-full hover:text-blue-600  group" @click="$emit('soportes')">
        <LinkIcon class="icon-style" />
        <p class="text-xs mt-0.5">
          {{ $t('soportes') }}
        </p>
      </button>
      <button data-tooltip-target="tooltip-home" type="button" class="text-gray-500 inline-flex flex-col items-center justify-center px-5 rounded-s-full hover:text-blue-600  group" @click="downloadPdfDirectlys">
